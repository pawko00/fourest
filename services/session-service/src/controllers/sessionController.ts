...
export const getTreeTypes = async (req: AuthRequest, res: Response) => {
  try {
    const treeTypes = await treeRepository.find({
      order: { unlockRequirement: "ASC" },
    });
    res.json(treeTypes);
  } catch (error) {
    console.error("Get tree types error:", error);
    res.status(500).json({ error: "Failed to get tree types" });
  }
};
...
async function updateUserStats(userId: string) {
  ...
  const lastSessionDate = lastSession ? new Date(lastSession.endedAt!) : null;
